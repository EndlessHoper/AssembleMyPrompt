...
export const UrlMention: React.FC<UrlMentionProps> = ({
  ...
  return (
    <span
      ...
      className={`inline-flex items-center px-1.5 py-0.5 rounded-md border transition-colors ${
        fileExists
    ...
    >
      <LinkIcon
        className={`inline-block h-3.5 w-3.5 mr-1 ${fileExists ? "text-green-500" : "text-yellow-500"}`}
        data-oid="38yxh36"
      />

      {element.url}
      {!fileExists && (
        <span className="text-xs ml-1 text-yellow-600" data-oid="e4o_e47">
          (fetching)
        </span>
      )}
      {children}
